...
def visualization(prediction):
    ...
    for i in range(len(y_pred_2)-1):
        if y_pred_2[i+1] > y_pred_2[i]:
            ax.plot([dates_day[i], dates_day[i+1]], [dates_day[i], dates_day[i+1]], color='green')
        else:
            ax.plot([dates_day[i], dates_day[i+1]], [dates_day[i], dates_day[i+1]], color='red')
    ...
    processed_data = visualization(prediction)
